...
@section('title', 'Select Subscription Products — Thikana')
...
<div class="sub-page-wrapper">
  ...
  <!-- Welcome Hero Banner -->
  <div class="sub-hero">
    <h1>Welcome to Thikana! 🎉</h1>
    <p>Your account has been created successfully.</p>
    <p class="sub-hero-note">Choose at least 3 products below and complete the ₹200 subscription to activate your account.</p>
  </div>

  @if (session('warning'))
    <div style="background: #fffaf0; border: 1px solid #fbd38d; color: #c05621; padding: 14px 18px; border-radius: var(--radius-md); margin-bottom: 24px; font-size: 14px; display: flex; align-items: center; gap: 10px;">
      <span>⚠️</span>
  ...
  @if ($errors->any())
    <div style="background: #fff5f5; border: 1px solid #feb2b2; color: #c53030; padding: 14px 18px; border-radius: var(--radius-md); margin-bottom: 24px; font-size: 14px;">
  ...
  <form id="productSelectionForm" action="{{ route('subscription.checkout') }}" method="POST">
    ...
    <div class="sub-sticky-bar">
      <div class="counter-box">
        <div id="counterBadge" class="counter-badge {{ count($selectedProductIds) >= 3 ? 'complete' : 'incomplete' }}">
          Selected: <span id="selectedCount">{{ count($selectedProductIds) }}</span> / 3
        </div>
        <div id="counterHint" class="counter-hint">
          @if (count($selectedProductIds) >= 3)
            ✓ Minimum met! You can proceed.
          @else
            Select at least {{ 3 - count($selectedProductIds) }} more product(s)
          @endif
        </div>
      </div>

      <div class="fee-summary">
        <div class="fee-amount">Subscription Fee: ₹{{ $subscriptionAmount }}</div>
        <button type="submit" id="btnContinue" class="btn-continue" {{ count($selectedProductIds) >= 3 ? '' : 'disabled' }}>
    ...
    <div class="products-grid">
      @foreach ($products as $product)
        ...
        <div class="product-card {{ $isSelected ? 'selected' : '' }}" data-id="{{ $product->id }}">
          <input type="checkbox" name="products[]" value="{{ $product->id }}" id="prod_{{ $product->id }}" {{ $isSelected ? 'checked' : '' }} style="display: none;" />
  ...
  <div class="modal-card">
    ...
    <div class="order-details">
      ...
      <div class="order-row">
        ...
        <span style="color: var(--green-deep);">₹200.00</span>
      </div>
    </div>
  ...
  document.addEventListener('DOMContentLoaded', function() {
    ...
    function updateSelectionState() {
      ...
      if (count >= 3) {
        ...
        counterHint.textContent = `Select at least ${3 - count} more product(s)`;
    ...
    form.addEventListener('submit', function(e) {
      ...
      if (selectedCheckboxes.length < 3) {
        alert('Please select at least 3 products before continuing.');
        return;
      }
