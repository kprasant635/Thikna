...
@section('title', 'Send Enquiry — ' . $business['name'] . ' — SkopX')
    ...
    <div class="wrap enq-wrap">
        ...
        <div class="enq-page-head">
            ...
            <p class="enq-sub">Your message goes directly to the verified owner. Typical response time:
                <strong>under 15 minutes</strong> during business hours.
            </p>
        </div>
        ...
        <div class="enq-layout">
            ...
            <section class="enq-form-col">
                ...
                <form class="enq-form" method="post" action="{{ route('business.quote', $business['slug']) }}"
                    ...
                    @error('need')
                        <span class="enq-error">{{ $message }}</span>
                    @enderror
                    ...
                    <div class="enq-field-row">
                        <div class="enq-field">
                            ...
                            <input type="text" id="enq-name" name="name" placeholder="e.g. Rohan Sharma"
                        ...
                        <div class="enq-field">
                            ...
                            <input type="tel" id="enq-phone" name="phone" placeholder="+91 98XXX XXXXX"
                    ...
                    <div class="enq-field">
                        ...
                        <select id="enq-product" name="product" required>
                            <option value="" disabled {{ old('product') ? '' : 'selected' }}>— Select an option —
                            </option>
                            @foreach ($business['quote_options'] ?? ['General inquiry', 'Product purchase', 'Repair / service', 'Bulk / wholesale order'] as $opt)
                                <option value="{{ $opt }}" {{ old('product') === $opt ? 'selected' : '' }}>
                                    {{ $opt }}</option>
                            @endforeach
                            <option value="other" {{ old('product') === 'other' ? 'selected' : '' }}>Other (describe
                                below)</option>
                        </select>
                        @error('product')
                            <span class="enq-error">{{ $message }}</span>
                    ...
                    <div class="enq-submit-row">
                        ...
                        <p class="enq-privacy">🔒 Your number is shared only with this verified business. Not sold to third
                            parties.</p>
                    </div>
                </form>
                ...
                <div class="enq-alt-contact">
                    ...
                    <div class="enq-alt-row">
                        ...
                        <a class="enq-alt-btn enq-alt-wa" href="{{ $business['whatsapp_url'] }}" target="_blank"
                            rel="noopener">
                            💬 WhatsApp chat
                        </a>
                        <a class="enq-alt-btn enq-alt-dir" href="{{ $business['directions_url'] }}" target="_blank"
                            rel="noopener">
                            🗺️ Get directions
                        </a>
                    </div>
            ...
            <aside class="enq-sidebar">
                    ...
                    <div class="enq-biz-info">
                        ...
                        <div class="enq-biz-badges">
                            @foreach ($business['badges'] as $badge)
                                <span
                                    class="badge {{ $badge['gold'] ?? false ? 'gold' : '' }}">{{ $badge['label'] }}</span>
                            @endforeach
                        </div>
                ...
                <div class="side-card enq-trust-card">
                    <h4>🛡️ Why enquire through SkopX?</h4>
                    <ul class="enq-trust-list">
                        ...
                        <li><span class="enq-trust-icon">⭐</span>Rated {{ $business['rating'] }} with
                            {{ number_format($business['review_count']) }} verified reviews</li>
                    </ul>
                </div>
                ...
                <div class="side-card enq-map-card">
                    ...
                    <a class="btn btn-outline enq-dir-btn" href="{{ $business['directions_url'] }}" target="_blank"
                        rel="noopener">
                        Open in Google Maps →
                    </a>
                </div>
