...
@section('title', 'Shop Directory — SkopX')
...
@section('content')
    ...
    <div class="breadcrumb-bar">
        <div class="wrap">
            ...
            <a href="{{ route('home') }}">{{ $city ?? 'Bhubaneswar' }}</a> /
    ...
    <div class="wrap" style="padding-top:22px;">
        <div class="results-head">
            <div>
                <h1>{{ count($shops) }} verified shops found near
                    <span style="color:var(--green-mid);">{{ $area ?? 'Downtown ' . ($city ?? 'Bhubaneswar') }}</span>
                </h1>
                <div class="results-count">{{ count($shops) }} active merchant nodes · avg lead response
                    {{ $avgResponseMins ?? 4 }} mins</div>
            </div>
            <form method="get">
        ...
        <div class="active-filters">
            @foreach ($activeFilters ?? ['Bhubaneswar Downtown (5 km)', 'Verified merchant'] as $f)
        ...
        <div class="results-layout">
            ...
            <aside class="filter-panel">
                ...
                <div class="filter-group">
                    ...
                    <div class="filter-option"><span><input type="checkbox" name="verified" checked> Verified only</span>
                    </div>
                ...
                <div class="filter-group">
                    ...
                    <div class="filter-option"><span><input type="radio" name="rating" value="4.5"> 4.5 &amp;
                            up</span></div>
                    <div class="filter-option"><span><input type="radio" name="rating" value="4.0"> 4.0 &amp;
                            up</span></div>
                </div>
                <div class="filter-group">
                    <div class="label">SEARCH RADIUS — {{ $radiusKm ?? 5 }} km</div>
                    <input type="range" min="1" max="25" value="{{ $radiusKm ?? 5 }}"
                        style="width:100%;accent-color:var(--green-bright);">
                </div>
                <div class="filter-group">
                    <div class="label">ESTABLISHMENT TYPE</div>
                    @foreach ($establishmentTypes ?? [] as $type => $n)
                        <div class="filter-option"><span>{{ $type }}</span><span
                                class="n">{{ $n }}</span></div>
                    @endforeach
                </div>
            </aside>
                ...
                @foreach ($shops as $shop)
                    <a class="merchant-card" href="{{ route('business.show', ['business' => $shop['slug']]) }}">
                        ...
                        <div class="merchant-actions">
                            ...
                            <div class="btn btn-{{ $shop['secondary_style'] ?? 'gold' }}">{{ $shop['secondary_action'] }}
                            </div>
                        </div>
                    </a>
                @endforeach
            ...
            <aside>
                ...
                <div class="side-card sidebar-live">
                    <h4>Live activity in {{ $city ?? 'Bhubaneswar' }}</h4>
                    @foreach ($liveActivity ?? [] as $item)
                        <div class="live-item"><b>{{ $item['name'] }}</b> {{ $item['action'] }} · {{ $item['time'] }} ago
                        </div>
                    @endforeach
                </div>
                <div class="side-card">
                    ...
                    @foreach ($areaCounts ?? [] as $area => $count)
                        <div class="hours-row"><span>{{ $area }}</span><span>{{ $count }} shops</span>
                        </div>
                    @endforeach
                </div>
            </aside>
