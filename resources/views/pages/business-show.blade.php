...
@section('title', $business['name'] . ' — SkopX')
...
@section('content')
    ...
    <div class="breadcrumb-bar">
        <div class="wrap">
            ...
            @if ($business['rank_badge'])
                <span class="badge" style="float:right;">{{ $business['rank_badge'] }}</span>
            @endif
        </div>
    </div>
    ...
    <div class="wrap">
        <div class="detail-header">
            <div class="detail-photo"><span class="official">{{ $business['photo_badge'] }}</span>{{ $business['icon'] }}
            </div>
            <div class="detail-info">
                ...
                <div class="detail-actions">
                    <a class="btn btn-primary" href="tel:{{ $business['phone'] }}">Call
                        {{ $business['phone_display'] }}</a>
            ...
            <section>
                        ...
                        <div class="product-card">
                            ...
                            <div class="product-body">
                                ...
                                <div class="product-foot">
                                    ...
                                    <a class="btn btn-tint"
                                        href="{{ route('business.enquiry', [$business['slug'], 'product' => $product['sku']]) }}">{{ $product['cta'] }}</a>
                                </div>
                            </div>
                        </div>
                    @endforeach
                ...
                <div class="review-summary">
                    ...
                    <div style="flex:1;">
                        @foreach ($ratingBreakdown as $stars => $pct)
                            <div class="bar-row">{{ $stars }}★<div class="bar-track">
                ...
                @foreach ($reviews as $review)
                    <div class="review-item">
                        <span class="who">{{ $review['name'] }} <span
                                class="badge">{{ $review['badge'] }}</span></span>
            ...
            <aside>
                <form class="quote-box" method="post" action="{{ route('business.quote', $business['slug']) }}">
                    ...
                    <div class="field"><label>Full name</label><input type="text" name="name"
                            placeholder="e.g. Jordan Miller"></div>
                    <div class="field"><label>Mobile number</label><input type="text" name="phone"
                            placeholder="+91 98XXX XXXXX"></div>
                    <div class="field"><label>Service or product needed</label>
                        <select name="need">
                            @foreach ($business['quote_options'] as $opt)
                                <option>{{ $opt }}</option>
                            @endforeach
                        </select>
                    </div>
                    <div class="field"><label>Notes</label>
                        <textarea name="notes" rows="3" placeholder="e.g. iPhone 14 Pro, cracked front glass"></textarea>
                    </div>
