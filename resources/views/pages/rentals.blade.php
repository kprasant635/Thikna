...
@section('title', 'Home Rentals — Thikana')
@section('minimal-footer', true)

@section('content')

<div class="wrap" style="padding-top:22px;">
  <div class="eyebrow-pill">🏠 {{ $city ?? 'Guwahati' }} verified rental marketplace</div>
  <div class="results-head">
    <h1>{{ $city ?? 'Guwahati' }} home rentals &amp; apartment listings</h1>
    <div class="results-count">{{ count($rentals) }}+ direct owner units · updated {{ $updatedAgo ?? '8 min' }} ago</div>
  </div>

  <form class="rental-search-bar" method="get">
    <div class="field"><label>Locality / Area</label><input type="text" name="area" value="{{ request('area', $area ?? 'North Guwahati') }}"></div>
...
<div class="wrap section" style="padding-top:0;">
  <div class="rentals-layout">
    ...
    <section>
      <div class="section-sub" style="margin-bottom:14px;">Showing {{ count($rentals) }} prime verified properties matching your preferences</div>

      @foreach($rentals as $rental)
      <a class="rental-item" href="{{ route('rentals.show', $rental['slug']) }}">
        <div class="rental-thumb"><span class="tag {{ $rental['tag_style'] ?? '' }}">{{ $rental['tag'] }}</span>
        ...
        <div class="rental-info">
          ...
          <div class="rental-price-row"><span class="amt">₹{{ number_format($rental['price']) }}</span><span class="per">/month · {{ $rental['deposit_label'] }}</span></div>
          <div class="rental-facts">
            @foreach($rental['facts'] as $fact)<span>{{ $fact }}</span>@endforeach
          </div>
          <div class="rental-tags">
            @foreach($rental['tags'] as $tag)<span>{{ $tag }}</span>@endforeach
          </div>
        </div>
    ...
    <aside>
      <div class="side-card">
        ...
        <p style="font-size:12.5px;color:var(--ink-soft);">Connect directly with owners and verified managers. Save on traditional finder commissions on every lease.</p>
      ...
      <div class="side-card" style="background:var(--green-tint);border:none;">
        ...
        <p style="font-size:12.5px;color:var(--ink-soft);">Reach thousands of monthly renters. Zero listing fees, verified tenant screening.</p>
        <a class="btn btn-gold" href="{{ route('business.create') }}" style="display:block;text-align:center;margin-top:10px;">List property now</a>
      </div>
      <div class="side-card">
        <h4>Average rent — {{ $city ?? 'Guwahati' }} (2 BHK)</h4>
        <div style="font-size:22px;font-family:'Fraunces',serif;color:var(--green-deep);">₹{{ number_format($avgRent ?? 15400) }}/mo</div>
