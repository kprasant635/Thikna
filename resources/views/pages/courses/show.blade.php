...
@section('title', $course->title . ' — Thikana')
...
<div class="player-layout">
  ...
  <div class="player-main">
    ...
    <div class="video-container" id="videoContainer">
      @if ($embedUrl)
        <iframe id="youtubeIframe" src="{{ $embedUrl }}" allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture" allowfullscreen></iframe>
      @else
    ...
    <div class="player-info">
      ...
      <div style="margin-top: 18px; padding-top: 14px; border-top: 1px solid var(--line); display: flex; justify-content: space-between; align-items: center; font-size: 13.5px; flex-wrap: wrap; gap: 10px;">
        <div id="videoProgressText" style="color: var(--ink-soft); font-weight: 600;">
          Status: {{ ($currentVideo->user_progress && $currentVideo->user_progress->is_completed) ? '✓ Completed' : 'In Progress' }}
        </div>
        <div style="display: flex; align-items: center; gap: 12px;">
          <button type="button" id="btnMarkWatched" class="btn btn-outline" style="padding: 6px 14px; font-size: 12.5px; border-radius: var(--radius-sm); border: 1.5px solid var(--green-deep); color: var(--green-deep); font-weight: 700;">
  ...
  <div class="playlist-card">
    <div class="playlist-header">
      ...
      <span style="font-size: 13px; color: var(--ink-soft); font-weight: 500;">{{ $videos->count() }} Videos</span>
    </div>
    ...
    <div class="playlist-items">
      @foreach ($videos as $index => $vid)
        ...
        <div class="playlist-item {{ $index === 0 ? 'active' : '' }} {{ $isVidCompleted ? 'completed' : '' }}"
             data-id="{{ $vid->id }}"
             data-url="{{ $vid->video_url }}"
             data-title="{{ $vid->title }}"
             data-desc="{{ $vid->description }}"
             data-duration="{{ $vid->duration_seconds }}"
          ...
          <div>
            ...
            <div class="playlist-item-duration">📺 YouTube Video • ⏱️ {{ round($vid->duration_seconds / 60, 1) }} mins</div>
          </div>
        </div>
      @endforeach
    </div>
    ...
    <div style="margin-top: 24px;">
      <a href="{{ route('courses.index') }}" class="btn btn-outline" style="width: 100%; text-align: center; padding: 10px; font-size: 13.5px; border-radius: var(--radius-sm); border: 1.5px solid var(--line);">
        ← Back to Courses
      </a>
    </div>
  </div>
...
<div id="completionModal" class="completion-modal">
  <div class="completion-card">
    ...
    <div style="display: flex; flex-direction: column; gap: 10px;">
      <a id="btnModalCertificate" href="{{ route('certificate.show') }}" class="btn btn-primary" style="padding: 12px; font-size: 15px; font-weight: 700; border-radius: var(--radius-sm); display: none;">
        🏆 View Official Certificate
      </a>
      <a href="{{ route('courses.index') }}" class="btn btn-outline" style="padding: 11px; font-size: 14px; border-radius: var(--radius-sm); border: 1.5px solid var(--line);">
...
<script>
  document.addEventListener('DOMContentLoaded', function() {
    ...
    function loadVideo(item) {
      ...
      if (ytId) {
        videoContainer.innerHTML = `<iframe id="youtubeIframe" src="https://www.youtube.com/embed/${ytId}?enablejsapi=1&autoplay=1" allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture" allowfullscreen></iframe>`;
      } else {
        videoContainer.innerHTML = `<video id="shortVideoPlayer" controls autoplay src="${url}"></video>`;
      }
    }
    ...
    function sendProgress(percent = 100) {
      ...
      .then(data => {
        ...
        if (data.is_all_courses_completed) {
          ...
          completionMessage.textContent = '🎉 Congratulations! You have successfully completed all 3 educational courses. Your official Thikana Certificate is ready!';
          completionModal.classList.add('active');
        } else if (data.is_course_completed) {
          completionMessage.textContent = 'Awesome! You have completed all videos in this masterclass. Continue to complete your remaining selected courses!';
          completionModal.classList.add('active');
        }
      })
