...
@section('title', 'My Educational Courses — SkopX')
...
@section('content')
    <div class="courses-wrapper">
        ...
        <div class="courses-hero">
            ...
            <p>Complete all 3 selected educational courses below to earn your official SkopX Certificate and unlock
                exclusive member benefits!</p>
        </div>

        @if (session('warning'))
            <div
                style="background: #fffaf0; border: 1px solid #fbd38d; color: #c05621; padding: 14px 18px; border-radius: var(--radius-md); margin-bottom: 24px; font-size: 14px;">
                ⚠️ {{ session('warning') }}
            </div>
        @endif
        ...
        @if (session('success'))
            <div
                style="background: #f0fff4; border: 1px solid #9ae6b4; color: #276749; padding: 14px 18px; border-radius: var(--radius-md); margin-bottom: 24px; font-size: 14px;">
                🎉 {{ session('success') }}
            </div>
        @endif
        ...
        <h2 style="font-size: 20px; font-weight: 700; margin-bottom: 18px; color: var(--ink);">My Selected Courses
            ({{ $courses->count() }})</h2>
        ...
        <div class="status-cards-row">
            ...
            <div class="status-box">
                <div class="status-box-icon"
                    style="background: {{ $is_all_completed ? '#f0fff4' : '#edf2f7' }}; color: {{ $is_all_completed ? 'var(--green-deep)' : '#a0aec0' }};">
                    {{ $is_all_completed ? '🏆' : '🔒' }}
                </div>
                <div class="status-box-content">
                    <h3>SkopX Official Certificate</h3>
                    @if ($is_all_completed && $certificate)
                        <p>Certificate #{{ $certificate->certificate_number }} Issued</p>
                        <a href="{{ route('certificate.show') }}" class="btn btn-primary"
                            style="padding: 8px 16px; font-size: 13.5px; border-radius: var(--radius-sm);">View Certificate
                            →</a>
                    @else
            ...
            <div class="status-box">
                <div class="status-box-icon"
                    style="background: {{ $user->isCertified() ? '#f0fff4' : '#edf2f7' }}; color: {{ $user->isCertified() ? 'var(--green-deep)' : '#a0aec0' }};">
                    {{ $user->isCertified() ? '🌟' : '🔒' }}
                </div>
                <div class="status-box-content">
                    <h3>Member Benefits</h3>
                    @if ($user->isCertified())
                        <p>All SkopX member benefits are fully unlocked!</p>
                        <a href="{{ route('benefits.index') }}" class="btn btn-outline"
                            style="padding: 8px 16px; font-size: 13.5px; border-radius: var(--radius-sm); border: 1.5px solid var(--green-deep); color: var(--green-deep);">Explore
                            Benefits →</a>
                    @else
